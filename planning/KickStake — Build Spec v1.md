...
- **Tiered draw (algo v2, the default in the UI):** each team carries a `strength_rank` — its ordinal position among the tournament's qualifiers (1 = strongest), seeded from the final pre-tournament FIFA world ranking. At draw time the rank-sorted teams are chunked into `floor(T/N)` bands of N; each band is shuffled (same seeded PRNG) and dealt one team per participant, so everyone gets exactly one team from every band — nobody lands two favourites while someone else gets two outsiders. Luck stays in play *within* a band. The `T mod N` remainder is the weakest teams (an extra team shouldn't be a windfall) and follows the remainder policy. The band is stored per assignment (`team_assignment.tier`) and the choice on the sweepstake (`draw_tiering`: `none` | `auto`); tournaments without ranks can only draw pure-random. Future option (designed for, not built): organiser-edited tiers passed as `tiers: teamId[][]`.
